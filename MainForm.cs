...
namespace PixelPatchStudio
{
    internal sealed partial class MainForm : Form
    {
        ...
        {
            Text = "PhotoSense 1.0.18 · Photoshop AI 局部重绘";
            Icon = AppIcon.Create();
        ...
        private Panel BuildGeneratePage()
        {
            ...
            creativeMode = new ModernCheckBox { Text = "创意模式（减少保护性约束，适合戏剧性光影）", Location = new Point(16, 446), Size = new Size(292, 28) };
            Label creativeModeHint = new Label
            {
                Text = "启用后使用精简提示词，让模型更自由地创作光影效果和氛围。\n✓ 如出现人物重影或肢体重叠，建议启用创意模式。",
            ...
            useEsrgan = new ModernCheckBox { Text = "生成后使用 Real-ESRGAN 超分", Location = new Point(16, 522), Size = new Size(292, 28) };
            installEsrganButton = ButtonOf("自动部署 Real-ESRGAN", false); installEsrganButton.SetBounds(16, 556, 292, 36); installEsrganButton.Click += InstallRealEsrgan;
            esrganState = new Label { Location = new Point(16, 598), Size = new Size(292, 42), ForeColor = UiTheme.Subtle, Font = new Font("Microsoft YaHei UI", 8f), AutoEllipsis = true };

            generateButton = ButtonOf("开始 AI 局部重绘", true); generateButton.SetBounds(16, 652, 292, 44); generateButton.Font = new Font("Microsoft YaHei UI", 10f, FontStyle.Bold); generateButton.Click += Generate;
        ...
        private async Task<GenerationBuild> GenerateVersionAsync(ImageApiClient client, Bitmap source, Bitmap mask, string key, int ordinal, int total, CancellationToken token)
        {
            ...
            Bitmap apiResult = await client.GenerateAsync(source, mask, referenceImage, prompt.Text, settings, key, creativeMode.Checked, token);
            try
            {
                string sizeInfo = string.Format("源图:{0}×{1} API返回:{2}×{3}", source.Width, source.Height, apiResult.Width, apiResult.Height);
                System.Diagnostics.Debug.WriteLine(sizeInfo);
                
                string resolutionWarning = ImageApiClient.GeminiResolutionWarning(settings, apiResult.Width, apiResult.Height);
                if (apiResult.Width != source.Width || apiResult.Height != source.Height)
                {
                    bool isSignificantDownscale = apiResult.Width < source.Width * 0.6 || apiResult.Height < source.Height * 0.6;
                    if (isSignificantDownscale)
                    {
                        string alignmentWarning = string.Format("API 返回尺寸 {0}×{1} 远小于源图 {2}×{3}。\n\n建议：\n1. 勾选下方 \"Real-ESRGAN 超分\" 提升清晰度\n2. 或在 Photoshop 中先缩小图片到 2000px 以内再处理", 
                            apiResult.Width, apiResult.Height, source.Width, source.Height);
                        if (string.IsNullOrEmpty(resolutionWarning)) resolutionWarning = alignmentWarning;
                        else resolutionWarning = alignmentWarning + "\n\n" + resolutionWarning;
                    }
                    else
                    {
                        string alignmentWarning = string.Format("API 返回尺寸 {0}×{1} 与源图 {2}×{3} 略有差异，已自动对齐。", 
                            apiResult.Width, apiResult.Height, source.Width, source.Height);
                        if (string.IsNullOrEmpty(resolutionWarning)) resolutionWarning = alignmentWarning;
                        else resolutionWarning = alignmentWarning + "\n\n" + resolutionWarning;
                    }
